...
public final class CropImage {
    ...
    private static final int MAX_REQ_CODE = 65000;
    private static final int MIN_REQ_CODE = 100;
     ...
     */
    public static void startPickImageActivity(@NonNull Activity activity, CropImageOptions mOptions) {
     ...
     */
    public static void startPickImageActivity(@NonNull Context context, @NonNull Fragment fragment, CropImageOptions mOptions) {
     ...
     */
    public static Intent getPickImageChooserIntent(@NonNull Context context, CropImageOptions mOptions) {
        return getPickImageChooserIntent(
                context, context.getString(R.string.pick_image_intent_chooser_title), mOptions.useGallery,false, mOptions.useCamera);
    }
    ...
    public static final class ActivityBuilder {
         ...
         */
        public int start(@NonNull Activity activity) {
            mOptions.validate();

            int reqCode = MIN_REQ_CODE + new Random().nextInt(MAX_REQ_CODE - MIN_REQ_CODE + 1);

            activity.startActivityForResult(getIntent(activity), reqCode);
         ...
         * @param activity activity to receive result
         */
        public int start(@NonNull Activity activity, @Nullable Class<?> cls) {
            mOptions.validate();

            int reqCode = MIN_REQ_CODE + new Random().nextInt(MAX_REQ_CODE - MIN_REQ_CODE + 1);

            activity.startActivityForResult(getIntent(activity, cls), reqCode);
         ...
         */
        public int start(@NonNull Context context, @NonNull Fragment fragment) {
            int reqCode = MIN_REQ_CODE + new Random().nextInt(MAX_REQ_CODE - MIN_REQ_CODE + 1);

            fragment.startActivityForResult(getIntent(context), reqCode);
        ...
        public int start(@NonNull Context context, @NonNull android.app.Fragment fragment) {
        ...
        public int start(
            ...
            fragment.startActivityForResult(getIntent(context, cls), reqCode);
        ...
        public ActivityBuilder setUseCamera(boolean useCamera) {
            mOptions.useCamera = useCamera;
            return this;
        }

        public ActivityBuilder setUseGallery( boolean useGallery) {
            mOptions.useGallery = useGallery;
            return this;
        }
